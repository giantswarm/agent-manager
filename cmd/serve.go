...
environment variable named next to it; flags win over the environment.`,
	...
	f.StringVar(&o.chartSemver, "agent-chart-semver", envOr("AGENT_CHART_SEMVER", agents.DefaultChartSemver), "Semver range the OCIRepository tracks; the default keeps agents on the 0.x line of the agent chart, below the 1.0.0 values schema (AGENT_CHART_SEMVER)")
