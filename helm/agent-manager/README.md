...
| oauth.trustedAudiences | list | `[]` | OAuth client IDs whose IdP id_tokens are accepted as bearer tokens (SSO token forwarding). Empty falls back to `[global.identity.clientId]`, the platform client muster's own sessions log in with. The audiences in `muster.mcpServer.auth.requiredAudiences` are trusted in addition, always: every token muster forwards carries them by construction and they are what the kube-apiserver trusts, so a session that signed in through another client — the portal's — is accepted too. Set this only for audiences beyond those two. |
